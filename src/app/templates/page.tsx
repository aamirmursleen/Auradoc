...
const TemplatesPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Floating Generate Button - Like template.net - ALWAYS VISIBLE */}
      <div className="fixed bottom-0 left-0 right-0 z-50 pointer-events-none">
        <div className="flex justify-center pb-6">
          <button
            onClick={() => {
              if (filteredTemplates.length > 0) {
                handleUseTemplate(filteredTemplates[0])
              }
            }}
            className="pointer-events-auto px-8 py-4 bg-gradient-to-r from-primary-600 to-primary-500 text-white rounded-full font-semibold shadow-2xl hover:shadow-3xl transition-all flex items-center gap-3 hover:scale-105 animate-bounce-slow"
          >
            <Sparkles className="w-5 h-5" />
            Generate My Free Document
          </button>
        </div>
        {/* Gradient fade effect */}
        <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-gray-100 to-transparent -z-10" />
      </div>
